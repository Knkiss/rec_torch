...
def calculate_performance_group(rank_new, i_num, group_num, rank, Ks, test_set, dataset, model):
    """
    计算分组性能
    @return: 分组性能
    """
    ...
    with tqdm(range(n_users), desc="Performance Calculate") as t:
...
def main(dataset, model, debug=False):
    ...
    result_grouped = calculate_performance_group(rank_new, i_num, group_num, rank, Ks, test_set, dataset, model)
    record_result(result, result_grouped, dataset, model, debug)
