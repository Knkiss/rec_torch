...
def show2dParameters(x, data, label, names, metric):
    ...
    plt.plot(x, data[0], marker='d', ms=10, lw=3, color=color_list[0], label="$\lambda_2 / \lambda_1 = 0.01$", ls='-.')
    plt.plot(x, data[1], marker='s', ms=10, lw=3, color=color_list[-1], label="$\lambda_2 / \lambda_1 = 0.05$")
    plt.plot(x, data[2], marker='o', ms=10, lw=3, color=color_list[1], label="$\lambda_2 / \lambda_1 = 0.1$", ls='--')
    ...
    plt.text(1, np.max(data[1]) + 0.002, '0.1519', family='monospace', fontsize=16, color=color_list[-1])
    plt.text(2, np.max(data[2]) + 0.002, '0.1517', family='monospace', fontsize=16, color=color_list[1])
    plt.title("AmazonBook", fontsize=18, weight='bold')  # 数据集标题

    # # movielens
    # plt.yticks([0.29, 0.3, 0.31, 0.32, 0.33, 0.34, 0.35], fontsize=16, weight='bold')
    # plt.text(1, np.max(data[1]) + 0.002, '0.3351', family='monospace', fontsize=16, color=color_list[1])
    # plt.text(2, np.max(data[2]) + 0.002, '0.3384', family='monospace', fontsize=16, color=color_list[-1])
    # plt.title("Movielens-1M", fontsize=18, weight='bold')  # 数据集标题

    # lastfm
    # plt.text(1.2, np.max(data[1]) + 0.001, '0.3860', family='monospace', fontsize=16, color=color_list[1])
    # plt.text(2, np.max(data[2]) + 0.002, '0.3870', family='monospace', fontsize=16, color=color_list[-1])
