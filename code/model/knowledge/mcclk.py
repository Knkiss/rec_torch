...
class MCCLK(model.AbstractRecModel):
    def __init__(self):
        super().__init__()
    ...
    def calculate_loss(self, users, pos, neg):
        ...
        loss = {
            Loss.BPR.value: rec_loss + self.reg_weight * reg_loss + self.beta * (self.alpha * local_loss + (1 - self.alpha) * global_loss)
        }
        return loss
...
class Aggregator(nn.Module):
    ...
    @staticmethod
    def calculate_sim_hrt(entity_emb_head, entity_emb_tail, relation_emb):
        r"""
...
class GraphConv(nn.Module):
    ...
    @staticmethod
    def edge_sampling(edge_index, edge_type, rate=0.5):
        # edge_index: [2, -1]
        # edge_type: [-1]
